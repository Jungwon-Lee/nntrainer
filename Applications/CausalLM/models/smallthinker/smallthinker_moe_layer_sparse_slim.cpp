 ...
 *
 * @file   smallthinker_moe_layer_sparse_slim.cpp
 * @date   30 June 2026
...
void SmallThinkerSparseSlimMoELayer::compute_expert_forward(
  const nntrainer::Tensor &input, nntrainer::Tensor &output,
  const std::vector<std::pair<unsigned, float>> &token_assignments,
  const nntrainer::Tensor &gate_proj, const nntrainer::Tensor &up_proj,
  const nntrainer::Tensor &down_proj, unsigned int hidden_size) {
  // The base class has already activated this expert's gate/up/down tensors;
  // run the shared sparse ReGLU FFN, accumulating into the shared `output`
  // (the base drives the expert loop serially).
