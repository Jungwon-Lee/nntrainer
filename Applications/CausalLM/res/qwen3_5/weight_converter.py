...
"""Convert dense Qwen3.5/Qwen3.6 text weights to NNTrainer format."""
...
def save_qwen3_5_for_nntrainer(params, config, dtype, output):
    """Write an in-memory HF dense model in NNTrainer FP32 allocation order."""
    ...
    def save_attention_layer(prefix):
        ...
        hidden = q.shape[1]
        expected = (2 * heads * head_dim, hidden)
        if tuple(q.shape) != expected:
            raise ValueError(f"unexpected q_proj shape: {tuple(q.shape)} != {expected}")
    ...
    for layer_id, layer_type in enumerate(layer_types):
        ...
        save(params[f"{prefix}.mlp.up_proj.weight"], transpose=True)
        save(params[f"{prefix}.mlp.gate_proj.weight"], transpose=True)
        save(params[f"{prefix}.mlp.down_proj.weight"], transpose=True)
...
def write_dense_mlp(writer, prefix):
    """Write dense SwiGLU in NNTrainer's up, gate, down order."""
    writer.write_q4_weight(f"{prefix}.mlp.up_proj.weight")
    writer.write_q4_weight(f"{prefix}.mlp.gate_proj.weight")
    writer.write_q4_weight(f"{prefix}.mlp.down_proj.weight")


def convert(model_dir, output_file, target_isa, chunk_rows):
    """Stream an official dense checkpoint directly into Q4_0."""
    ...
    with ShardedTensorStore(model_dir) as store, output_file.open("wb") as output:
        ...
        for layer_id, layer_type in enumerate(layer_types):
            ...
            write_dense_mlp(writer, layer_prefix)
...
def main():
    ...
    model_name = re.sub(r"[^a-z0-9._-]+", "-", model_dir.name.lower())
    output_file = output_dir / f"nntr_{model_name}-q4_0-{args.target_isa}.bin"
