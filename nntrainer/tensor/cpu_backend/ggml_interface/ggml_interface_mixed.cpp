...
bool __ggml_q4_0_4x8_q8_0_GEMV_MASKED(const unsigned int N,
  ...
  if (!nntr_gemv_q4_0_4x8_q8_0_sparse_supported() || N % 4 != 0 ||
      K % QK8_0 != 0)
    return false;
  ...
  if (active_count * 2 > N)
    return false;
  if (!nntr_should_use_output_masked_q4_0(output_mask, N))
    return false;
  if (active_count == 0) {
    std::fill(C, C + N, 0.0f);
    return true;
  ...
  thread_local std::vector<char> QA;
  QA.resize(sizeof(block_q8_0) * blocks_per_row);
  nntr_quantize_row_q8_0(A, QA.data(), K);

  constexpr unsigned int tile_size = 4;
  ...
  tm.parallel_for(0, thread_num, [=](size_t thread_idx) {
    ...
    nntr_gemv_q4_0_4x8_q8_0_output_masked(
